...
def main() -> None:
    ...
    met_old = compute_image_metrics(B_test, B_old_hat_test, data_range=1.0)
    met_new = compute_image_metrics(B_test, B_new_hat_test, data_range=1.0)
    ...
    summary = {
        ...
        'baseline': {
            'fidelity_mse_train': float(fidelity_mse(B_train, baseline.B_hat)),
            'sym_defect': float(symmetry_defect(T_old, J_A, J_B)),
            'ssim_mean_test': float(met_old.ssim_mean),
            'psnr_mean_test': float(met_old.psnr_mean),
        },
        'equivariant': {
            'fidelity_mse_train': float(fidelity_mse(B_train, sol_new.B_hat)),
            'sym_defect': float(symmetry_defect(T_new, J_A, J_B)),
            'ssim_mean_test': float(met_new.ssim_mean),
            'psnr_mean_test': float(met_new.psnr_mean),
            'cg_info': {
                'info_code': int(cg_info.info_code),
                'n_iter': int(cg_info.n_iter),
                'final_resid_norm': float(cg_info.final_resid_norm),
            },
        },
    }
    ...
    save_histogram(met_old.ssim, outdir / 'ssim_old.png', title='SSIM (old)')
    save_histogram(met_new.ssim, outdir / 'ssim_new.png', title='SSIM (new)')
    save_histogram(met_old.psnr, outdir / 'psnr_old.png', title='PSNR (old)')
    save_histogram(met_new.psnr, outdir / 'psnr_new.png', title='PSNR (new)')
    ...
    save_image_grid(
        B_old_hat_test[:25].reshape(-1, 28, 28),
        outdir / 'recon_old_grid.png',
        title='Reconstruction (old)',
    )
    save_image_grid(
        B_new_hat_test[:25].reshape(-1, 28, 28),
